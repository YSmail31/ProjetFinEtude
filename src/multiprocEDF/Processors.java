...
public class Processors {
    ...
    public boolean DMfirstFit(){
        ...
        {
            int i = 0;
            while(i < nbrProc && !processors.get(i).isEDFSchedulable(t))
                i++;
    ...
    public boolean DMnextFit(){
        ...
        {
            int i = nextFit;
            while(i != nextFit && !processors.get(i).isEDFSchedulable(t))
                i = (i+1)%nbrProc;
    ...
    public boolean DMbestFit(){
        ...
        for(Task t : taskset.getTaskSet())
        {
            ...
            for(i = 0 ; i < nbrProc ; i++)
            {
                ...
                if(minUtil > utilisation && processors.get(i).isEDFSchedulable(t))
                {
    ...
    public boolean DMworstFit(){
        ...
        for(Task t : taskset.getTaskSet())
        {
            ...
            for(i = 0 ; i < nbrProc ; i++)
            {
                ...
                if(maxUtil < utilisation && processors.get(i).isEDFSchedulable(t))
                {
                    existSchedulable = true;
                    maxUtil = utilisation;
                    indProc = i;
    ...
    void insertionLocale() {
        ...
        {
            Task tsl = proc.taskset.getSleepTask();
            if(tsl.getC() > 0)
            {
                proc.addTask(tsl);
            }
