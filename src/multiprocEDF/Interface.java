...
public class Interface extends javax.swing.JFrame {
    ...
    private void jButton6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton6ActionPerformed
        JFileChooser jf=new JFileChooser();
        jf.setCurrentDirectory(new File("."));
        jf.addChoosableFileFilter(new FileNameExtensionFilter("Latex", "tex", "tex"));
        jf.setFileFilter(new FileNameExtensionFilter("Latex", "tex", "tex"));
        jf.setAcceptAllFileFilterUsed(false);
        int res=jf.showSaveDialog(this);
        if(res == JFileChooser.APPROVE_OPTION)
        {
            saveAsPath = jf.getSelectedFile().getAbsolutePath();
            if(!saveAsPath.endsWith(".xml"))
                saveAsPath += ".xml";
