...
public class Interface extends javax.swing.JFrame {
    ...
    private void jButton8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton8ActionPerformed
        ...
        InterfacePrincipale.Interface.rms.setVisible(false);
    ...
    private String footerTiks(double x,double y){
        String s = "\\draw [->](0,0) -- coordinate (x axis mid) ("+(y*10+0.5)+",0);\n" +
                "\\draw [->](0,0) -- coordinate (y axis mid) (0,"+(x*10+0.5)+");\n" + 
    ...
    private void gen(int type) throws IOException {
        ...
        String boxEff = "",boxUtil= "";
        int nbex = p.nbtu;
        int fin = (int)(p.mu*10);
        int nbprc = p.nbp;
        int algo = p.algo;
        barg.setValue(0);
        FileWriter fwe = new FileWriter(saveAsPath+".data");
        FileWriter fwu = new FileWriter(saveAsPath+"u.data");
        for(int j=1;j<fin;j++)
        {
            System.out.println(j/10.0);
            System.out.println("----");
            double efficacitéMoyenne = 0;
            double utilisationMoyenne = 0;
            for(int i = ((j-1)*nbex); i < (j*nbex) ; i++)
            {
                ...
                utilisationMoyenne += processors.getU();
            }
            efficacitéMoyenne /= nbex;
            utilisationMoyenne /= nbex;
            ...
            barg.setValue(j);
            fwe.write(j+"\t"+(efficacitéMoyenne*10)+"\n");
            fwu.write(j+"\t"+(utilisationMoyenne*10)+"\n");
        }
        boxEff += "\\draw plot[mark=*, mark options={fill=white}] \n" + "file {?};\n";
        boxUtil += "\\draw plot[mark=triangle*, mark options={fill=red}] \n" + "file {?};\n";
        String nomFichier = saveAsPath.substring(saveAsPath.lastIndexOf("\\")+1);
        boxEff = boxEff.replaceFirst("[?]",nomFichier+".data");
        boxUtil = boxUtil.replaceFirst("[?]",nomFichier+"u.data");
        fwe.close();
        fwu.close();
        ...
        try {
            ...
            fw.write(headerTiks());
            fw.write(boxEff);
            fw.write(footerTiks(1,p.mu+0.1));
            fw.write(headerTiks());
            fw.write(boxUtil);
            fw.write(footerTiks(p.nbp,p.mu+0.1));
            fw.write(footerTex());
            fw.close();
        } catch (IOException ex) {
            Logger.getLogger(Interface.class.getName()).log(Level.SEVERE, null, ex);
